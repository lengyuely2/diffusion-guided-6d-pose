...
class Instance_Segmentation_Model(pl.LightningModule):
    def __init__(
        ...
        **kwargs,
        ...
        env_skip = os.environ.get("ISM_SKIP_EXISTING_NPZ", "").lower() in (
            "1",
            "true",
            "yes",
        )
        self.skip_existing_predictions = bool(skip_existing_predictions or env_skip)
        try:
            self.empty_cuda_cache_every = int(
                os.environ.get("ISM_EMPTY_CUDA_EVERY", str(empty_cuda_cache_every))
            )
        except ValueError:
            self.empty_cuda_cache_every = empty_cuda_cache_every
        if self.skip_existing_predictions:
            logging.info(
                "skip_existing_predictions=True：已存在的 scene*_frame*.npz 将跳过（断点续跑）"
            )
    ...
    def test_step(self, batch, idx):
        ...
        npz_path = file_path + ".npz"
        if self.skip_existing_predictions and osp.isfile(npz_path):
            if idx % 2000 == 0:
                logging.info("skip existing (%d): %s", idx, npz_path)
            if (
                self.empty_cuda_cache_every > 0
                and (idx + 1) % self.empty_cuda_cache_every == 0
            ):
        ...
        # save detections to file（file_path 与上文 skip 检查一致）
