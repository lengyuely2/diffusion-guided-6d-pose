...
# Headless without NVIDIA EGL: set BOP_VISPY_APP=osmesa (uses Mesa libOSMesa, slower but works on WSL).
_app = os.environ.get("BOP_VISPY_APP", "egl")
if _app == "osmesa":
...
# app backends: glfw, pyglet, egl, osmesa
# gl backends: gl2, pyopengl2, gl+  (do not pass gl= for osmesa — vispy requirement)
app_backend = _app
gl_backend = "gl2"  # "pyopengl2"  # speed: 'gl+' < 'gl2' < 'pyopengl2'
if app_backend == "osmesa":
    vispy.use(app=app_backend)
    print("vispy uses app: {} (software OSMesa; set BOP_VISPY_APP=egl for GPU EGL)".format(app_backend))
else:
