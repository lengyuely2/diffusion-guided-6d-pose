#!/usr/bin/env python3
"""
加载 train.py 保存的 eps 网络（12 维 x12），对单帧做 DDPM 采样，并与 BOP GT 对比。
"""
...
from diff.geometry import rotation_geodesic_deg, x12_to_pose
...
def main() -> int:
    ...
    pd = int(ckpt.get("pose_dim", 12))
    if pd != 12:
        print("此脚本用于 pose_dim=12；vec9 请用 sample_pose6d.py", file=sys.stderr)
        return 1
    model = EpsMLP(
        pose_dim=12,
    ...
    inst = instances[li]
    x0_gt = torch.from_numpy(inst.pose_x12.copy()).unsqueeze(0).float()
    ...
    print(
        f"x12 L2 vs GT: mean={float(np.mean(errs)):.4f}  std={float(np.std(errs)):.4f}  "
        f"(n={len(errs)} runs)"
    )
