#!/usr/bin/env python3
"""与 train.py 相同，但 pose 为 vec9（6D 旋转 + t/500）。"""
...
def main() -> None:
    ...
    ap.add_argument(
        "--split",
        default="test",
        help="BOP ycbv 下 {split}_metaData.json，如 test、train_real（大数据常用，需磁盘上确有该 json）",
    )
    ...
    ap.add_argument("--save", type=Path, default=Path("diff/output/eps_mlp_pose6d.pt"))
    ...
    ap.add_argument(
        "--ism-crop-cond",
        action="store_true",
        help="追加 ISM 与 GT 匹配后的实例裁剪 RGB（8×8，+192 维），建议与 --require-ism 同开",
    )
    ap.add_argument(
        "--obj-emb-dim",
        type=int,
        default=0,
        help=">0 时用 nn.Embedding 编码物体 id（不写进 cond 向量）；与 --cond-obj-id 互斥",
    )
    ap.add_argument(
        "--log-file",
        type=Path,
        default=None,
        help="额外写日志到该文件（行缓冲，适合 nohup + conda run 时立刻 tail）",
    )
    args = ap.parse_args()
    if args.log_file is not None:
        setup_tee_log(args.log_file)
        print(f"[train_pose6d] tee log -> {args.log_file.resolve()}", flush=True)
    ...
    if include_ism_crop and not args.require_ism and args.resume is None:
        print("[train_pose6d] 警告: --ism-crop-cond 通常需要 --require-ism", flush=True)
